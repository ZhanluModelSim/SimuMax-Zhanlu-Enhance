...
class PerfLLM(PerfBase):
    ...
    def _cross_sanity_check(self) -> bool:
        ...
        if self.strategy.cp_size > 1 and self.strategy.cp_comm_type == "a2a":
            ...
            # MQA-per-rank models (dense FA branch holds one KV head per rank,
            # so the model-level kv_head_num need not divide cp_size) skip the
            # Ulysses-style KV-head-split check. Declared via config flag
            # `mqa_per_rank`, not a model_type string special-case.
            if (self.model_config.kv_head_num is not None
                    and not getattr(self.model_config, 'mqa_per_rank', False)):
